...
namespace Brokerage.Worker.Messaging.Consumers
{
    public class AddBlockchainToBrokerAccountConsumer : IConsumer<AddBlockchainToBrokerAccount>
    {
        ...
        public async Task Consume(ConsumeContext<AddBlockchainToBrokerAccount> context)
        {
            ...
            if (!unitOfWork.Outbox.IsClosed)
            {
                ...
                unitOfWork.Outbox.Send(new UpdateBrokerAccountAccounts()
                {
    ...
    public class UpdateBrokerAccountAccountsConsumer : IConsumer<UpdateBrokerAccountAccounts>
    {
        private readonly ILoggerFactory _loggerFactory;
        private readonly IUnitOfWorkManager<UnitOfWork> _unitOfWorkManager;
        private readonly IVaultAgentClient _vaultAgentClient;
        private readonly IAccountsRepository _accountsRepository;
        private readonly ILogger<UpdateBrokerAccountAccountsConsumer> _logger;

        public UpdateBrokerAccountAccountsConsumer(ILoggerFactory loggerFactory,
            IUnitOfWorkManager<UnitOfWork> unitOfWorkManager,
            IVaultAgentClient vaultAgentClient,
            IAccountsRepository accountsRepository)
        {
            _loggerFactory = loggerFactory;
            _logger = _loggerFactory.CreateLogger<UpdateBrokerAccountAccountsConsumer>();
            _unitOfWorkManager = unitOfWorkManager;
            _vaultAgentClient = vaultAgentClient;
            _accountsRepository = accountsRepository;
        }

        public async Task Consume(ConsumeContext<UpdateBrokerAccountAccounts> context)
        {
            var command = context.Message;
            ...
            await using var unitOfWork = await _unitOfWorkManager
                .Begin($"BrokerAccounts:UpdateBrokerAccountAccounts:{command.BrokerAccountId}-{idempotencyIdPart}");
